...
8755];
    ...
    println!("How many measurements are larger than the previous measurement? {}", count);

    // Part 2
    // Sum a three sample sliding window and a second three sample sliding window
    // Count the number of times a sliding window depth measurement increases from the previous
    // sliding window
...
// Part 2: Count three value sliding sums where the depth increases.
fn sum_last_three(x: &[i32], index: usize) -> i32 {
    return x[index] + x[index - 1] + x[index - 2];
}
...
mod tests {
    ...
    #[test]
    fn test_measure_2_increases() {
        let measurements: [i32; 3] = [199,
                                200,
                                208];

        assert_eq!(count_increases(&measurements), 2);
    }

    #[test]
    fn test_measure_7_increases() {
    ...
    fn test_sum3() {
        ...
        assert_eq!(sum_last_three(&measurements, 2), 607);
        assert_eq!(sum_last_three(&measurements, 3), 618);
        assert_eq!(sum_last_three(&measurements, 4), 618);
        assert_eq!(sum_last_three(&measurements, 5), 617);
        assert_eq!(sum_last_three(&measurements, 6), 647);
        assert_eq!(sum_last_three(&measurements, 7), 716);
        assert_eq!(sum_last_three(&measurements, 8), 769);
        assert_eq!(sum_last_three(&measurements, 9), 792);

    }

    #[test]
    fn test_average_increases() {
        let measurements: [i32; 10] = [199,
                                200,
                                208, // 607
                                210, // 618
